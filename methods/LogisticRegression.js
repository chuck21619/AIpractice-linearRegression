class LogisticRegression {
    ...
    constructor(chart) {
        ...
        this.trainModelAndGraphData = function (json, initialCallback, finishedCallback) {
            ...
            this.trainModel(scaled_inputs, targets, (results) => {
                ...
                var equationString = keys.map((item, index) => {
                    ...
                    return '1 / (1 + e^-(((' + item + ' - ' + this.feature_average.toFixed(2) + ')/' + this.feature_range.toFixed(2) + ')*' + this.weight.toFixed(2)
                }).join(' + ');
        ...
        this.computeGradient = function computeGradient(x, y, w, b) {
            ...
            const dj_dz = predictions.map((prediction, index) => { return (prediction - y[index]) });
            const dj_dw = dj_dz.reduce((accumulator, dl_dz, index) => { return accumulator + dl_dz*x[index] }, 0) / m;
            const dj_db = dj_dz.reduce((accumulator, dl_dz)        => { return accumulator + dl_dz          }, 0) / m;         
            return [dj_dw, dj_db];
        }
        ...
        this.predict = function predict(inputs, weight, bias) {
            return inputs.map(input => this.sigmoid((input * weight + bias)));
        }
